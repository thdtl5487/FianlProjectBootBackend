...
public class NBoardRepository{
   ...
   public void update(NBoardVO vo) {
      nboardRepository.findById(vo.getBNum());
      vo.setBtitle(vo.getBtitle());
      vo.setBtext(vo.getBtext());
      vo.setBregDate(vo.getBregDate());
      nboardRepository.save(vo);
   }
   

   public void delete(NBoardVO vo) {
      nboardRepository.deleteById(vo.getBNum()); // em.remove : JPA를 통해 값을 제거할 때
   }
   ...
   public ResponseEntity<Map> getBoard(Long bnum) {
      ...
      Optional<NBoardVO> getBoard = null;
      getBoard = nboardRepository.findById(bnum);
